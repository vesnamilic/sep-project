...
public class SellerController {
	...
	public ResponseEntity<?> addSeller(@RequestBody Seller seller) {
		...
		if(newSeller != null) {
			logger.info("COMPLETED | Adding a new PaymentHub seller to the PayPal database | Email: " + seller.getEmail());
			return ResponseEntity.status(201).build();
		}
		else {
			logger.error("CANCELED | Adding a new PaymentHub client to the PayPal database | Email: " + seller.getEmail());
